# Data Dictionary — DDRAtlas

**Dataset:** DNA-damage repair gene alterations across the TCGA PanCancer Atlas (32 tumour types, n = 10,195).
**Source:** cBioPortal public REST API (v7.0.0, retrieved June 2026). **Genes (20):** BRCA1, BRCA2, PALB2, RAD51, RAD51C, ATM, ATR, CHEK1, CHEK2, MLH1, MSH2, MSH6, PMS2, ERCC1, ERCC2, XPA, PARP1, FANCA, PRKDC, TP53BP1.
...
## 1. `tcga_pancan_dna_damage_repair_alteration_freqs_long.csv` — master long table (640 rows × 23 cols)
...
20 gene rows × 33 columns = `geneSymbol` index + 32 tumour-type columns; each cell is the frequency for that gene in that tumour (any-alteration / CNA-only / mutation-only respectively).

## 5. `dna_damage_repair_gene_rank_summary.csv` (20 rows × 9 cols)
...
## 6. `dna_damage_repair_top5_tumours_per_gene.csv` (100 rows × 9 cols)
`geneSymbol`, `rank` (1–5), `tumour_acronym`, `N`, `any_alter_freq`, `mut_freq`, `any_cna_freq`, `amp_freq`, `deep_del_freq`.

## 7. `dna_damage_repair_tumour_burden_ranking.csv` (32 rows × 7 cols)
`rank` (1–32), `tumour_acronym`, `mean_any_alter_freq`, `mean_mut_freq`, `mean_any_cna_freq` (means across all 20 genes), `N`, `n_genes` (=20). Ranked by `mean_any_alter_freq` (desc).

## 8. `brca_dna_damage_repair_alteration_frequencies.csv` (20 rows × 15 cols; N = 1052)
...
## 10. `tcga_low_coverage_tumours_for_dna_damage_repair.txt`
